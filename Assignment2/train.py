    ...
    def kl_anneal_function(anneal_function, step):
        ...
        if anneal_function == 'linear':
            ...
            if(weight > 1):
                weight = 1
            return weight
        if anneal_function == 'sqrt':
            weight = math.sqrt(step) * 0.011
            if(weight > 1):
                weight = 1
            return weight
    ...
    for epoch in range(args.epochs):
        ...
        for split in splits:
            ...
            for iteration, batch in enumerate(data_loader):
                ...
                # loss calculation
                recon_loss, KL_loss, KL_weight = loss_fn(logp, batch['target'].long(),
                ...
                # bookkeepeing
                tracker['negELBO'] = torch.cat((tracker['negELBO'], torch.tensor([loss.data]).cuda()))
